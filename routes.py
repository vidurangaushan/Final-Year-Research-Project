...
def get_quizzes(collection, page=1, per_page=10):
    ...
    try:
        ...
        quizzes = [
            ...
            for quiz in collection.find().skip(skip).limit(per_page)
        ]
...
def handle_quiz_requests():
    ...
    try:
        ...
        if subject == "maths":
         ...
         # Get page and per_page values from query parameters (optional)
        page = int(request.args.get("page", 1))  # Default to page 1
        per_page = int(request.args.get("per_page", 10))  # Default to 10 quizzes per page

        # Call the get_quizzes function with pagination arguments
        return get_quizzes(collection, page, per_page)
