...
export function Maths() {
    ...
    const {id} = useParams();
    const[mathsquiz, setMathsquiz] = useState([]);
    ...
    useEffect(()=>{
        function getMathsquiz(){           
          axios.get("http://127.0.0.1:5000/maths").then((res)=>{
                console.log(res);
                setMathsquiz(res.data);
                setLoading(false);
    ...
    const handleSubmit = () =>{
      // Check if all questions have been answered
        const unansweredQuestions = mathsquiz.filter((quiz, index) => {
      ...
      const submittedAnswers = mathsquiz.map((quiz, index)=>{
        const selectedAnswer = document.querySelector(`input[name="question_${index}"]:checked`);
        return{
  ...
  return (
    ...
    <div
      ...
      {mathsquiz && mathsquiz.map(m => (
      <div class="card" style={{marginLeft:20, margin:20, backgroundColor:"#FAFAAA",borderRadius: 20}}>
        <div key={index}>
            <h3>{m.question}</h3>
            <ul>
              {m.answers.map((answer, answerIndex) => (
                <li key={answerIndex}>
    ...
    <div className="content">
      {loading ? ( // Render loading message while data is being fetched
          ...
          mathsquiz && mathsquiz.length > 0 ? (
              mathsquiz.map((m, index) => (
                  <div className={`py-10 p-6 font-short-stack rounded-3xl shadow-md mb-2 bg-white ${borderColor}`} key={index} style={{width:1000,marginLeft:250,marginTop: 20,padding:10, backgroundColor: "#FAFAAA", borderRadius: 20 }}>
                      <div className='mb-4'>
                        <lable className = "block text-xl font-semibold mb-2">
                          ...
                          <ul>
                              {m.options && m.options.map((option, optionIndex) => (
                                  <li key={optionIndex}>
